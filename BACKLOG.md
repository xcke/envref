...
- [TODO] P1 | ENV-014 | Add `envref get <KEY>` command — print single resolved value
...
- [TODO] P1 | ENV-030 | Define `SecretBackend` interface (`Get(key) → value`, `Set(key, value)`, `Delete(key)`, `List()`)
- [TODO] P1 | ENV-031 | Implement backend registry with ordered fallback chain (try backend 1 → 2 → 3)
- [TODO] P1 | ENV-032 | Namespace secrets per project (`<project>/<key>`) to avoid collisions

### 3b — OS Keychain Backend

- [TODO] P1 | ENV-033 | Implement macOS Keychain backend via `go-keyring`
- [TODO] P1 | ENV-034 | Implement Linux `libsecret` / `secret-service` backend via `go-keyring`
- [TODO] P1 | ENV-035 | Implement Windows Credential Manager backend via `go-keyring`
...
- [TODO] P2 | ENV-040 | Implement local vault backend (SQLite + age encryption at `~/.config/envref/vault.db`)
...
- [TODO] P1 | ENV-060 | Add `envref secret set <key>` — prompt for value (hidden input), store in backend
...
- [TODO] P2 | ENV-064 | Add `envref secret set <key> --value <val>` — non-interactive mode for scripting
...
- [TODO] P2 | ENV-104 | Add `envref validate --ci` — exit code 1 on failure for CI pipelines
...
- [TODO] P2 | ENV-110 | Add shell completions (`envref completion bash|zsh|fish`)
